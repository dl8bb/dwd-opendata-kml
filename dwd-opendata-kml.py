...
def toJson(dateStr, timeStamp, rad1h, ff, neff):
    if (debug): print("toJson(%s, %s, %s, %s)" % (dateStr, timeStamp, rad1h, ff))
    result = {
        "Date":                dateStr,
        "Timestamp":           timeStamp,
        "Rad1h":               float(rad1h),
        ...
        "Windspeed":           round(ff, 3),
        "EffectiveCloudCover": round(neff, 3),
    }

    return result

def updateInflux(region, timeStamp, data):
    if (debug): print("updateInflux(%s %s %s)" % (region, timeStamp, json.dumps(data)))
...
#    print("Time (", len(ele_TimeStamp), ") ", ele_TimeStamp)
    ...
    if (debug): print("Rad1h (", len(ele_Rad1h), ") ", ele_Rad1h)

    ele_FF = getElementValueAsList(tree, 'FF')
    if (debug): print("FF (", len(ele_FF), ") ", ele_FF)
...
#
    ...
    if (debug): print("Neff (", len(ele_Neff), ") ", ele_Neff)
#
